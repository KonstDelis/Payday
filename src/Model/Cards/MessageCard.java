 ...
 * a MessageCard and the methods to access and transform the data
 * @author csd4623
 *
 */
public class MessageCard implements Card {
	...
	int cardNumber;
	/**
	 * <b>constructor</b>: Constructs a new MessageCard with the given
     * parameter info and cardNumber<br>
     * <b>Precondition</b>: info is not null and contains all the necessary data for the card
     * and cardNumber is between 0 and 47<br>
     * <b>postcondition</b>: Creates and initializes a MessageCard with the given
     * info (which is a String[] with the Title, type, value,
     * imagePath, message and 1 choice of the card) and cardNumber 
     * (the number of the order of the card on the board). <br>
	 * @param info String[] with the Title, type, value,
     * imagePath, message and 1 choice of the card
	 * @param cardNumber the number of the order of the card on the board
	 */
	public MessageCard(String[] info, int cardNumber) {
		if(info==null) throw new NullPointerException("info cannot be null");
		if(info.length!=6) throw new InvalidParameterException("The info doesn't contain all the necessary data for the card");
		if(cardNumber>=48||cardNumber<0) throw new InvalidParameterException("card number must be between 0 and 47");
		this.info = info;
	 ...
	 * 
	 * <b>Precondition</b>: for mail Cards 0&lt;=number&lt;=47<br>
	 * 
	...
	public int getValue() {
		...
		try {
			...
			System.err.println("Info[4] unable to be parsed to an int");
		}
		return value;
	}
	/**
	...
	public String toString() {
		String out = "Type: "+getType()+", Value: "+getValue()+", CardNumber: "+getCardNumber()+
